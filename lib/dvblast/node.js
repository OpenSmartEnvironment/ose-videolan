...
var Ucast = O.class('./ucast');
var Mcast = O.class('./mcast');
...
exports.spawn = function(entry, config, cb) {  // {{{2
  if (entry.ps) {
    var ps = entry.ps;
    delete entry.ps;
    ps.kill();
  ...
  Tmp.file(function(err, path, fd) {
    ...
    Fs.close(fd, function(err) {
      ...
      entry.ps.on('error', psError.bind(entry, entry.ps));
  ...
  Dvb.readPledge(e, req, socket, function(player, mcast, mplex, channel) {
    if (mcast && (mcast === e.mcast)) {  // Check mcast {{{3
      ...
      case 'mcast':  // Streaming in mcast mode
        ...
        return;
      default:
        throw O.error(e, 'Invalid `dvbStream` value', e.dvbStream);
      }
    }
...
function psError(ps, err) {  // {{{2
//  console.log('DVBLAST ERROR', arguments);

  O.log.error(err);

  psExit.apply(this, arguments);
}
...
function psExit(ps, code, signal) {  // {{{2
  ...
  var err = O.error(this, 'PROCESS_EXIT', 'DVBStream process exited', {pid: ps.pid, code: code, signal: signal});
  if (! Array.isArray(c)) {
    O.link.error(c, err);
    return;
  }
