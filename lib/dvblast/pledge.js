 ...
 */
  ...
  function doit() {  // {{{3
    ...
    if (entry.pledge) {  // Serialize multiple pledge requests
      entry.once('afterPledge', doit);
      return;
    }
    entry.pledge = that;
    ...
    if (
      entry.current.pledges.length === 1 &&
      O._.isEqual(req.playback, entry.current.pledges[0].playback)
    ) {
      buildAim();
      return;
    }
  ...
  function buildAim() {  // {{{3
    ...
    function onView(err, resp) {  // {{{4
      ...
      for (var i = 0; i < resp.view.length; i++) {
        counter.inc();
        aim.mplex.shard.get(resp.view[i], onChannel);
      }
  ...
  function open(aim) {  // {{{3
    ...
    if (aim) {
      that.aim = true;
      entry.aim.pledges.push(that);
      resp.count = entry.aim.pledges.length;
      resp.ucast = entry.aim.ucast;
    } else {
      entry.current.pledges.push(that);
      resp.count = entry.current.pledges.length;
...
exports.close = function(req) {  // {{{2
  Kind.removePledge(this.entry, this);
};
