 ...
 */
  ...
  Ose.link.open(this, socket, {score: 0});
};
 ...
 */
  ...
  spawn(this.entry, req.channel, req.ucast.dest, function(err) {
    if (err) {
      Ose.link.error(socket, err);
    } else {
      Ose.link.close(socket, {url: e.data.url});
    }
  });
};
...
// }}}1
// Private {{{1
function spawn(entry, channel, address, cb) {  // {{{2
  var mplex;
  ...
  function onKillall() {  // {{{3
    Fs.writeFile('/tmp/dvblast.cfg', address + ' 1 ' + channel.data.number, onFile);
  }
  ...
  function onFile(err) {  // {{{3
    ...
    var args = [
      '--adapter', 0,
      '--bandwidth', mplex.data.bandwidth,
      '--frequency', mplex.data.freq,
      '--modulation', mplex.data.modulation,
