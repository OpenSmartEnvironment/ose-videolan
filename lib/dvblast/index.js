...
// Public {{{1
exports.mplex2Config = function(entry, cb) {  // {{{2
  ...
  function onChannel(err, resp) {  // {{{3
    ...
    entry.mcast.post('getIp', undefined, function(err, ip) {
      if (err) {
        counter.dec();
        return;
      }
  ...
  function done() {  // {{{3
    var c = [];
    for (var key in r) {
      c.push(r[key].address + ':5004/epg 1 ' + key);
    }
...
exports.cleanDvb = function(entry, kill) {  // {{{2
  delete entry.dvbClients;
  delete entry.dvbStream;
  delete entry.dvbMplex;
  delete entry.dvbChannels;
  delete entry.dvbConfig;
