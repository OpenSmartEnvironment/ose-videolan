...
//      '--http-host', '10.166.25.8',
      '--http-port', '4439',
      '--http-user', 'ose',
      '--http-password', 'brekeke',  // TODO: generate password, or use some configured one
      '--no-auto-preparse',
      '--no-video-title-show',
    ];
    ...
    */
    ...
    handle.stderr.once('data', onStderr);
    ...
    function onStderr(data) {  // {{{
      ...
      if (data.indexOf('VLC media player') === 0) {
...
function setProp(entry, socket, name, val) {  // {{{2
  if (entry.props) {
    console.log('SET PROPERTY', name, val);
