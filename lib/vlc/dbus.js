...
function onSeeked(val) {  // {{{2
  if (! (this.state.pos && this.state.pos.length)) return;
...
function readMetadata(that, data) {  // {{{2
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    case 'vlc:nowplaying':
      ...
      break;
    case 'mpris:length':
      if (value <= 0 || value >= 18446744073709552000) {
      ...
      break;
    default:
      O.log.unhandled('VLC metadata', {key: key, value: value});
    }
  }
};
...
*/
  ...
  function connectVlc(pid) {  // {{{3
    ...
    var svc = that.bus.getService(name);
